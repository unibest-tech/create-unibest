...
export const UI_LIBRARY_CONFIGS: Record<UILibrary, UILibraryConfig | null> = {
    ...
    'skyiee-ui': {
        packageName: 'skyiee-ui',
        // skyiee-ui 的配置需要根据实际文档补充
        easycom: {
            pattern: '^sky-(.*)',
            path: 'skyiee-ui/components/$1/$1.vue',
        },
    },
}
 ...
 * 应用 UI 库配置到项目
 */
export async function applyUILibraryConfig(
    projectPath: string,
    uiLibrary: UILibrary,
): Promise<void> {
    // 如果选择的是 none，直接返回
...
async function updatePagesConfig(
    projectPath: string,
    easycom: { pattern: string; path: string },
): Promise<void> {
    ...
    if (match) {
        ...
        if (updatedCustom) {
            // 如果已有内容，确保格式正确
            updatedCustom = updatedCustom.endsWith(',')
                ? `${updatedCustom}\n${newConfig}`
                : `${updatedCustom},\n${newConfig}`
        } else {
        ...
        content = content.replace(
            easycomRegex,
            `easycom: {${beforeCustom}${updatedCustom}${afterCustom}}`,
        )
    } else {
